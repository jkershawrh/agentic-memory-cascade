...
class TestTitleAndDescription:
    ...
    def test_h1_starts_with_action_verb(self, readme_lines):
        ...
        action_verbs = [
            ...
            "Handle", "Know", "Let", "Make", "Predict", "Prevent", "Protect",
            "Prove", "Reduce", "Resolve", "Solve", "Stop", "Surface",
            "Compress",
        ]
...
class TestRequiredSections:
    ...
    def test_architecture_diagram_exists(self, readme_text):
        has_mermaid = "```mermaid" in readme_text
        has_text_diagram = "```text" in readme_text or "```\n" in readme_text
        has_image = bool(re.search(r"!\[.*?\]\(.*?\)", readme_text))
        has_docs = (DOCS / "architecture.md").exists() or (DOCS / "cascade-architecture.png").exists()
        assert has_mermaid or has_text_diagram or has_image or has_docs, (
            "No architecture diagram found"
        )
